...
void drvCan_init(uint32_t baudrate, uint16_t filter)
{
    ...
    CAN1->MCR = 0x8000;  // master reset
    while ((CAN1->MSR & 0x2) == 0);    // wait for sleep acknowledge
    CAN1->MCR |= 1;      // start init mode
    while ((CAN1->MSR & 0x1) == 0);    // wait for init acknowledge
    ...
    CAN1->BTR = prescaler | (bitSegment1<<16) | (bitSegment2<<20);

    CAN1->sFilterRegister[0].FR1 = (filter<<5) | 0xffe00000;
    CAN1->FA1R = 1;

    CAN1->MCR = 0x4;      // stop init mode
    while ((CAN1->MSR & 0x1) != 0);    // wait for init acknowledge
}
...
bool drvCan_sendMessage(uint16_t canId, uint8_t dlc, const uint8_t *data)
{
    ...
    {
        if ((CAN1->sTxMailBox[i].TIR & 1) == 0)
        {
            CAN1->sTxMailBox[i].TDLR = dataLow;
            CAN1->sTxMailBox[i].TDHR = dataHigh;
            CAN1->sTxMailBox[i].TDTR = dlc;
            CAN1->sTxMailBox[i].TIR = (uint32_t)canId << 21 | 1;
            tranmitted = true;
            break;
        }
    }
